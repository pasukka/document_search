...
async def remove_files(call: CallbackQuery, button: Button, dialog_manager: DialogManager):
    ...
    try:
        ...
        await message.answer("Произошла ошибка во время удаления файла.")
    finally:
...
async def get_data(**kwargs):
    ...
    bot_logger.info(f"Chat: {id} - Showing files list.")
    id = 1
    files = []
...
async def download_docs(message: types.Message, bot: Bot, file_name: str):
    ...
    bot_logger.info(
        f"Chat: {message.chat.id} - Got file {file_name} in chat.")
