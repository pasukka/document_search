...
class DocumentSearcher:
    ...
    def _summarize_user_intent(self, query: str) -> str:
        ...
        self.searcher_logger.info("Summarized user intent")
        if self.debug:
            self.searcher_logger.debug(f"INTENT: {intent}")
        return intent

    def _get_context(self, user_intent: str) -> list[str]:
        documents_context_list = self.context_retriever(user_intent)
        self.searcher_logger.info("Got context from document database")
        if self.debug:
            self.searcher_logger.debug(
    ...
    def _make_str_chat_history(self) -> str:
        ...
        self.searcher_logger.info("Made str from chat_history.")
    ...
    def ask(self, query: str) -> str:
        ...
        try:
            if self.debug:
                self.searcher_logger.debug(f"QUERY: {query}")
            ...
            self.error_code = 1
            self.searcher_logger.error("RunTime error.")
            self.searcher_logger.exception(e)
            raise TimeError() from e
        except ValueError as e:
            self.error_code = 2
            self.searcher_logger.error("Token expired.")
            self.searcher_logger.exception(e)
    ...
    def restart(self) -> None:
        ...
        self.searcher_logger.info(f"Restarting doc searcher.")
